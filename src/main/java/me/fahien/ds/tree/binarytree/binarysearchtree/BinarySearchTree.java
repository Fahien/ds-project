...
public class BinarySearchTree<Key, Value> extends LinkedBinaryTree<Entry<Key, Value>> implements Dictionary<Key, Value> {
	...
	@Override public Iterable<Entry<Key, Value>> findAll(Key key) throws InvalidKeyException {
		...
		for (Entry<Key, Value> entry : getEntries()) {
	...
	/** Returns the entry at this position */
	protected Entry<Key, Value> entry(Position<Entry<Key, Value>> position) {
		return checkPosition(position).getElement();
	}

	/** Returns the key of the entry at this position */
	protected Key key(Position<Entry<Key, Value>> position) {
		return entry(position).getKey();
	}

	/** Returns the value of the entry at this position */
	protected Value value(Position<Entry<Key, Value>> position) {
		return entry(position).getValue();
	}

	/** Returns the entry of this position with this new one */
	protected void replaceEntry(Position<Entry<Key, Value>> position, Entry<Key, Value> entry) {
		checkPosition(position).setElement(checkEntry(entry));
	}

	/** Adds all entries having this key of the subtree of this entry to this list */
	protected void addAll(PositionList<Entry<Key, Value>> list, Position<Entry<Key, Value>> entry, Key key) {
		if (isExternal(entry)) {
			return;
		}
		Position<Entry<Key, Value>> position = treeSearch(key, entry);
		if (!isExternal(position)) {
			addAll(list, left(position), key);
			list.addLast(position.getElement());
			addAll(list, right(position), key);
		}
	}
}